...
class AdaptiveFusionModule(nn.Module):
    ...
    def forward(self, rgb_features, depth_features):
        ...
        print(f"squeeze_tensor_rgb:", squeeze_tensor_rgb.shape)
        squeeze_tensor_depth = self.squeeze(depth_features)
        print(f"squeeze_tensor_depth:", squeeze_tensor_depth.shape)

        # 使用卷积层得到每个通道的权重（RGB 和 Depth 分开）
        excitation_rgb = self.fc1(squeeze_tensor_rgb)
        print(f"excitation_rgb before relu:", excitation_rgb.shape)
        excitation_rgb = self.relu(excitation_rgb)
        print(f"excitation_rgb after relu:", excitation_rgb.shape)
        excitation_rgb = self.fc2_rgb(excitation_rgb)  # 生成RGB特征的权重
        print(f"excitation_rgb before sigmoid:", excitation_rgb.shape)
        excitation_rgb = self.sigmoid(excitation_rgb)
        print(f"excitation_rgb:", excitation_rgb.shape)
    ...
    def forward(self, x):
        ...
        if self.confidence_map is not None:
            ...
            for i in range(4):
                ...
                print(f"confidence_maps[{i}]:", confidence_maps[str(i)].shape)
        ...
        for name, module in self.items():
            ...
            if 'rgb' in name:
                ...
                if layer_name in self.return_layers:
                    ...
                    print(f"rgb_out[{out_name}]:", rgb_out[out_name].shape)
            # if 'depth' in name:
            ...
            #         depth_out[out_name] = depth_x
                ...
                if layer_name in self.return_layers:
                    ...
                    if self.confidence_map is not None and confidence_map is not None:
                        ...
                        depth_out[out_name] = depth_x
                    print(f"depth_out[{out_name}]:", depth_out[out_name].shape)
            # elif 'rgb' in name:
            #     rgb_x = module(rgb_x)
            #     if layer_name in self.return_layers:
            #         out_name = self.return_layers[layer_name]
